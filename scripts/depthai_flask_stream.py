        ...
        try:
            ...
            while True:
                ...
                if conf.useCamera:
                    ...
                    tmp=pv.show_frames(callback=show_frames_callback)
                    print(cnt)
                    cnt+=1
                    (flag, encodedFrame) = cv2.imencode(".jpg", tmp)
                    if(a==1):
...
#Just some part of code is changed like creating the above part in function, yielding the frames and the below flask part ,else other is same
...
def video_feed():
    return Response(main_run(),mimetype='multipart/x-mixed-replace; boundary=frame')
